...
public class GifGenerator
{
        ...
        CancellationToken token)
    {
        ...
        // Render frames from 0 to 360 in increments of 10
        int totalFrames = 37; // 0, 10, 20, ... 360
        ...
        for (int i = 0; i <= 360; i += 10)
        {
            ...
            // Render this frame
            var result = await rayTracer.RenderAsync(scene, frameSettings, null, token);
            
            if (result.pixels != null)
            {
                Texture2D frame = new Texture2D(result.width, result.height);
                frame.SetPixels(result.pixels);
                frame.Apply();
                frames.Add(frame);
            }
        }
    ...
    /// <summary>
    /// Encode frames to GIF and save to file
    /// </summary>
